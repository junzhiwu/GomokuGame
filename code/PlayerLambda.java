...
public class PlayerLambda extends CKPlayer {
    ...
    public PlayerLambda(byte player, BoardModel state)
    {
        super(player, state);
        mentalMap = new ExistingSegments();
		opponent = (player==((byte)1)?(byte)2: (byte)1);

        teamName = "Gomoku Player Lambda";
    }
...
//        double value = 0;
//        
//        for (existingSegmentsBeta.segment i: mentalMap.getList()){
//            value = Math.pow(5,i.getLength()) * Math.pow(3,i.highThreatNum(state)) * Math.pow(2,i.lowThreatNum(state));
//
...
//       	 }
     ...
     if(state.getLastMove() != null){
        ...
        for (Point p: EmptyTiles)//for each empty tile,
            {
                ...
                {   
                    ...
                    {
                    	ExistingSegments.mapTile(p, mentalMap, player);
                    	return p;
                    }
                    ...
                    int value = EvaluateBoard(clone, cloneMap, player);		//Evaluate the board. The board will output 2^30 or 2^-30 if it finds a winning move.
                    
                    TileValues.set(EmptyTiles.indexOf(p), (Integer) value);	//Record the value of that piece

                    if (value == win)
                    {
                        return p;			//Any winning move will be instantly played. ExistingSegmentsBeta may be ignored since the player instantly wins.
                    }
                  ...
                  //Now check to make sure that your opponent will not win on the next turn. (This is only done on the first depth, to save time)
                    clone = state.placePiece(p, opponent);    
                    if (clone.winner() == opponent)          //In the event that player 2 can win on their next turn,
                    {
                        screwed = true;                //Set a flag that explains that if there are no winning moves,
                        sabotage = new Point(p);      //then the AI for player 1 MUST move here to prevent losing.(If there are two winning moves, the AI loses anyway.)
                    }
                }
            }
        
        //And after searching for winning moves, if it happened to find a losing one, Then it has to move there to prevent losing.
        if (screwed)             
        {
            if (EmptyTiles.size() == 1)
            {
            	ExistingSegments.mapTile(sabotage, mentalMap, player);
                return sabotage; //No need to panic, this is THE LAST TILE AND IT'S YOUR TURN.
            }
            
            for(Point p: EmptyTiles) //The AI now desperately searches for a move that prevents the opponent from winning in one turn. This supercedes iterative deepening search
            {           
                Boolean test_bit = true;
                BoardModel HOPE_THIS_WORKS = state.placePiece(p, player);
                for (Point j : EmptyTiles)
                {
                    if (EmptyTiles.indexOf(p) != EmptyTiles.indexOf(j))
                    {
                        BoardModel DID_IT_WORK = HOPE_THIS_WORKS.placePiece(j,opponent);
                        if (DID_IT_WORK.winner() == opponent)
                        {
        ...
        System.out.println("LAMBDA - Current Point: " + BestPoint + " Current Value: " + BestValue + " at Depth 1 (Node Searches Pruned: 0) Time: " + (int)(System.currentTimeMillis() - startTime));
        ...
        while ((int)(System.currentTimeMillis() - startTime) < deadline - 100 && depth <= Size)
        {
        	...
        	if (new_value_in)
        	{
        		...
        		best_depth = depth;
        		best_prunes = prunes;
        		System.out.println("LAMBDA - Current Point: " + CurrentBestPoint + " Current Value: " + CurrentBestValue + " at Depth " + best_depth + " (Node Searches Pruned: " + best_prunes + ") Time: " + (int)(System.currentTimeMillis() - startTime));
        	}
        ...
        System.out.println("LAMBDA - Best Point: " + CurrentBestPoint + " Best Value: " + CurrentBestValue + " Max Depth: " + best_depth);
...
//        	System.out.println("TimeAfter : " + (int)(System.currentTimeMillis() - startTime));
    ...
    {         
    	...
    	int openThree = 0;		//A Three with all threats free, create open four    - - O O O - -  
    	...
    	for (ExistingSegments.segment i: map.getList()){
    		...
    		if (current_player == opponent){
    			...
    			if (i.belongsTo() == opponent){
    				if (i.getLength() == 5) return Integer.MIN_VALUE;
    				if (i.getLength() == 4 && i.highThreatNum(state) > 1) return Integer.MIN_VALUE +1;
    				if (i.getLength() == 4 && i.highThreatNum(state) == 1) return Integer.MIN_VALUE + 1;
    				if (i.getLength() == 3 && i.highThreatNum(state) > 1 && i.lowThreatNum(state) > 0) return Integer.MIN_VALUE + 20;
    				if (i.getLength() == 2 && i.comboThreat(state)) return Integer.MIN_VALUE + 300;
    				
    				valueSum -= 4 * Math.pow(4,i.getLength()) * Math.pow(3,i.highThreatNum(state)) * Math.pow(2,i.lowThreatNum(state));	
    			}
    			if (i.belongsTo() == player){
    				if (i.getLength() == 5) return Integer.MAX_VALUE;
    				if (i.getLength() == 4 && i.highThreatNum(state) > 1) valueSum = Integer.MAX_VALUE/3;
    				if (i.getLength() == 4 && i.highThreatNum(state) == 1) valueSum = Integer.MAX_VALUE/3;
    				if (i.getLength() == 3 && i.highThreatNum(state) > 1 && i.lowThreatNum(state) > 0) valueSum =  Integer.MAX_VALUE/4;
    				if (i.getLength() == 2 && i.comboThreat(state)) valueSum = Integer.MAX_VALUE/5;
    				
    				valueSum += Math.pow(4,i.getLength()) * Math.pow(3,i.highThreatNum(state)) * Math.pow(2,i.lowThreatNum(state));	
    			}
    		}
    		...
    		if (current_player == player){ 
    			...
    			if (i.belongsTo() == player){
    				...
    				valueSum += 2 *  Math.pow(4,i.getLength()) * Math.pow(3,i.highThreatNum(state)) * Math.pow(2,i.lowThreatNum(state));	
    			} 
    		}    	
    		if (current_player == player){
	    		...
	    		// There are two   - O O O O -
	    		if (openFourOpposing >= 1) return Integer.MIN_VALUE + 2;
	    		if (openFour >=1 ) return Integer.MAX_VALUE - 2;
	
	    		// There is X O O O O - and a - - O O O - - 
	    		//Forced defensive move allows building of open four
	    		if (singleFourOpposing >=1 && openThreeOpposing >= 1) return Integer.MIN_VALUE + 20;
	    		if (singleFour >=1 && openThree >= 1) return Integer.MAX_VALUE - 20;
	    		
	    		//There is a  - O O - O - and X O O O O -
	    		//Forced defense allows building of open four
	    		if (comboOpposing >= 1 && singleFourOpposing >= 1) return Integer.MIN_VALUE + 20;
	    		if (combo >= 1 && singleFour >= 1) return Integer.MAX_VALUE - 20;

	    		//There is a - O O - O O and - - O O O - - 
	    		//Allows building of open four
	    		if (comboOpposing >= 1 && openThreeOpposing >= 1) return Integer.MIN_VALUE + 20;
	    		if (combo >= 1 && openThree >= 1) return Integer.MAX_VALUE - 20;
	    		
	    		//There is - O O - O - and - O O - O -
	    		//Allows building of open four
	    		if (comboOpposing > 1) return Integer.MIN_VALUE +20;
	    		if (combo > 1) return Integer.MAX_VALUE -20;

	    		//There is - - O O O - - and - - O O o - -
	    		//Allows building of open four
	    		if (openThreeOpposing >= 2) return Integer.MIN_VALUE + 20;
	    		if (openThree >= 2) return Integer.MAX_VALUE - 20;
	    		
	    		//Allows threat of an open four, no more
	    		if (comboOpposing >= 1) return Integer.MIN_VALUE + 300;
	    		if (combo >= 1) return Integer.MAX_VALUE -300;
	
	    		//	    		//*****************************************************
//	    		//Need to reorder these more optimally
//	    		
//	    		//There is an open three - - O O O - -
//	    		//Distance from win is one more than open 4 = 2
//	    		if (openThreeOpposing >= 1) return Integer.MIN_VALUE + 300;
//	    		if (openThree >= 1) return Integer.MAX_VALUE -300;	
//	
//	    		//There are two X O O O O -
//	    		//Opponent can only defend against one
//	    		//Distance from win is 2 (1 opponent 1 player move)
//				if (singleFourOpposing >= 2) return Integer.MIN_VALUE + 2;
//	    		if (singleFour >= 2) return Integer.MAX_VALUE - 2;
//				
//	    		//There are two O O O - O
//	    		//Opponent can only defend against one
//	    		//Distance from win is 2 (1 opponent and 1 player move)
//	    		if (combo5Opposing > 1) return Integer.MIN_VALUE +2;
//	    		if (combo5 > 1) return Integer.MAX_VALUE -2;
//	    		
//	    		//There is O O O - O and - - O O O - -
//	    		//Distance from win is 4 (2 opponent and 2 player moves)
//	    		if (combo5Opposing >= 1 && openThreeOpposing >= 1) return Integer.MIN_VALUE + 20;
//	    		if (combo5 >= 1 && openThree >= 1 ) return Integer.MAX_VALUE - 20;
//	    		
//	    		//There is O O O - O and X O O O O -
//	    		//Distance from win is 2 (1 opponent and 1 player move)
//	    		if (combo5Opposing >= 1 && singleFourOpposing >= 1) return Integer.MIN_VALUE + 20;
//	    		if (combo5 >= 1 && singleFour >= 1) return Integer.MAX_VALUE - 20;
