...
public class PlayerNu extends CKPlayer {
    ...
    public PlayerNu(byte player, BoardModel state)
    {
        super(player, state);
        mentalMap = new ExistingSegments();
		opponent = (player==((byte)1)?(byte)2: (byte)1);

        teamName = "Gomoku Player Nu";
    }
...
//        System.out.println("Nu - Current Point: " + BestPoint + " Current Value: " + BestValue + " at Depth 1 (Node Searches Pruned: 0) Time: " + (int)(System.currentTimeMillis() - startTime));
        ...
        System.out.println("Nu - Best Point: " + CurrentBestPoint + " Best Value: " + CurrentBestValue + " Max Depth: " + best_depth);
...
//        	//If none of the above then we have an evaluation situation:
    		...
    		if (openFourOpposing > 0 && combo5 + singleFour + openFour == 0) return Integer.MIN_VALUE + 1;
    		if (openFour > 0 && combo5Opposing + singleFourOpposing + openFourOpposing == 0) return Integer.MAX_VALUE -1;
        	
    		opponentVal *= Math.pow(16,(singleFourOpposing + combo5Opposing));
        	opponentVal *= Math.pow(128, openFourOpposing);
        	opponentVal *= Math.pow(127, (openThreeOpposing + comboOpposing));
        	
        	playerVal *= Math.pow(16,(singleFour + combo5));
        	playerVal *= Math.pow(64, openFour);
        	playerVal *= Math.pow(32, (openThree + combo));
...
//        	if (openThree + combo > 0 && singleFourOpposing + combo5Opposing == 0) return Integer.MAX_VALUE-2;
    		...
    		if (openFour > 0 && combo5Opposing + singleFourOpposing + openFourOpposing == 0) return Integer.MAX_VALUE;
    		if (openFourOpposing > 0 && combo5 + singleFour + openFour == 0) return Integer.MIN_VALUE +1;
    		
        	opponentVal *= Math.pow(16,(singleFourOpposing + combo5Opposing));
        	opponentVal *= Math.pow(64, openFourOpposing);
        	opponentVal *= Math.pow(32, (openThreeOpposing + comboOpposing));
        	
        	playerVal *= Math.pow(16,(singleFour + combo5));
        	playerVal *= Math.pow(128, openFour);
        	playerVal *= Math.pow(31, (openThree + combo));
